<div class="container-fluid">
	...
	<div class="clearfix mb-3">
		<div class="float-left">
			<p class="h2 mt-4 pull-left text-danger"><?php echo $page_heading; ?> &nbsp; ( <?php echo RUPEE_SYMBOL." ".$total_expense; ?> )</p>
		</div>
		
		<div class="float-right mt-4">
			<a href="<?php echo base_url("expense/add"); ?>" class="btn btn-primary text-white">Add</a>
		</div>
	</div>
				...
				<tbody>
					<?php
						...
						<tr class="
							...
							expense_date_<?php echo $row['expense_date']; ?>
						">
							...
							<td>
								<span class="badge active-badge <?php echo ($row['status']==STATUS_ACTIVE)?"badge-success activeinactive_show":"activeinactive_hide"; ?>" table-name="expense" set-status="<?php echo STATUS_INACTIVE; ?>" record-id="<?php echo $row['id']; ?>" ask-for="<?php echo $row['title']; ?>">
									<?php echo $arrStatus[STATUS_ACTIVE]; ?>
								</span>
								<span class="badge inactive-badge <?php echo ($row['status']==STATUS_INACTIVE)?"badge-danger activeinactive_show":"activeinactive_hide"; ?>" table-name="expense" set-status="<?php echo STATUS_ACTIVE; ?>" record-id="<?php echo $row['id']; ?>" ask-for="<?php echo $row['title']; ?>">
							...
							<td>
								<?php
									...
									$edit_page_url = base_url("expense/edit?".$edit_page_param);
								?>
								<a href="<?php echo $edit_page_url; ?>" data-toggle="tooltip" data-placement="top" title="edit">
									<i class="fa fa-edit"></i>
								</a>
								
								<a class="ml-2 delete-button text-danger" href="javascript:;" data-toggle="tooltip" data-placement="top" title="delete" table-name="expense" set-status="<?php echo STATUS_DELETE; ?>" record-id="<?php echo $row['id']; ?>" ask-for="<?php echo $row['title']; ?>">
